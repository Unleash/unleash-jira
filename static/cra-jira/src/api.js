...
const fetchFeatureToggle = async (issueKey) => {
  const res = await fetch(await getToggleUrl(issueKey), await getAuth());
  if (res.ok) {
...
const createFeatureToggle = async (toggleData) => {
  ...
  const data = await fetch(await getCreateUrl(), {
...
const fetchProjects = async () => {
  const unleashApiKey = await getApiKey();
  const data = await fetch(await getProjectUrl(), {
    headers: { Authorization: unleashApiKey },
  });
  if (data.ok) {
...
const fetchUiBootstrap = async () => {
  const unleashApiKey = await getApiKey();
  const data = await fetch(await bootstrapUrl(), {
    headers: { Authorization: unleashApiKey },
  });
